...
export default function ServiceSoftwareProcess() {
  const [processSteps, setProcessSteps] = useState([]);

  useEffect(() => {
    const fetchProcess = async () => {
      try {
        const records = await pb.collection('page_software_process').getFullList({ sort: '+step_number' });
        setProcessSteps(records);
      } catch (err) {
        console.error("Failed to load software process:", err);
      }
    };
    fetchProcess();
  }, []);
        ...
        <ScrollReveal className="mb-16 lg:mb-24">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-12 lg:gap-24 border-b border-slate-200 pb-16">
            ...
            <p className="text-slate-500 font-light max-w-sm text-lg leading-relaxed lg:text-right shrink-0">
              A structured, transparent engineering pipeline designed to launch your custom software securely and at scale.
            </p>
          </div>
        </ScrollReveal>
