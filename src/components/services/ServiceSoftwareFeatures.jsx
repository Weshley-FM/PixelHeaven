...
export default function ServiceSoftwareFeatures() {
  const [features, setFeatures] = useState([]);

  useEffect(() => {
    const fetchFeatures = async () => {
      try {
        const records = await pb.collection('page_software_features').getFullList({ sort: '-created' });
        setFeatures(records);
      } catch (err) {
        console.error("Failed to load software features:", err);
      }
    };
    fetchFeatures();
  }, []);
  ...
  return (
    <section className="bg-slate-900 text-white py-32 md:py-48 relative selection:bg-white selection:text-slate-900">
      ...
      <div className="max-w-[90vw] 2xl:max-w-[1400px] mx-auto px-6 mt-16 md:mt-24">
        ...
        {/* Clean, Divided 4-Column Grid matching exactly the style of UI/UX Products */}
